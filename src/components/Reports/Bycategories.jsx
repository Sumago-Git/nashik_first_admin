...
const Bycategories = () => {
  ...
  const fetchData = async (category) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/search/search?categories=${category}`);
      const result = await response.json();
      setData(result.years); // Set the fetched data
    } catch (error) {
  ...
  return (
    <div>
      <h2>5-Year Slot and Booking Form Summary with Filters</h2>
      ...
      <Dropdown>
        ...
        <Dropdown.Menu>
          {/* School */}
          <Dropdown.Item onClick={() => setSelectedCategory("School Students Training – Group")}>School</Dropdown.Item>

          {/* Adult Dropdown with sub-options */}
          <Dropdown.ItemText>Adult</Dropdown.ItemText>


          <Dropdown.Menu >
            <Dropdown.Item onClick={() => setSelectedCategory("RTO – Training for School Bus Driver ,RTO – Suspended Driving License Holders Training ,RTO – Learner Driving License Holder Training")}>Adult</Dropdown.Item>

            <Dropdown.Menu style={{ marginLeft: "100px" }}>
              <Dropdown.Item onClick={() => setSelectedCategory("RTO – Learner Driving License Holder Training")}>RTO – Learner Driving</Dropdown.Item>
              <Dropdown.Item onClick={() => setSelectedCategory("RTO – Suspended Driving License Holders Training")}>Suspended Driving</Dropdown.Item>
              <Dropdown.Item onClick={() => setSelectedCategory("RTO – Training for School Bus Driver")}>Bus Driver    </Dropdown.Item>
              <Dropdown.Item onClick={() => setSelectedCategory("College/Organization Training – Group")}>College/Organization</Dropdown.Item>
      ...
      {/* Filters */}
      <div style={{ marginBottom: "20px", display: "flex", gap: "10px" }} className="d-flex justify-content-end">
        {/* Year Filter (Multi-select using react-select) */}
        <Select
          isMulti
          options={yearOptions}
          value={yearFilter.map((year) => ({ label: year, value: year }))}
          onChange={(selectedOptions) =>
            setYearFilter(selectedOptions.map((option) => option.value))
          }
        ...
        {/* Month Filter (Multi-select using react-select) */}
        <Select
          isMulti
          options={monthOptions}
          value={monthFilter.map((month) => ({ label: month, value: month }))}
          onChange={(selectedOptions) =>
            setMonthFilter(selectedOptions.map((option) => option.value))
          }
        ...
        {/* Week Filter (Multi-select using react-select) */}
        <Select
          isMulti
          options={weekOptions}
          value={weekFilter.map((week) => ({ label: week, value: week }))}
          onChange={(selectedOptions) =>
            setWeekFilter(selectedOptions.map((option) => option.value))
          }
                ...
                >
                  <Container>
                    <Row>
                      ...
                      <Col lg={4}>{" "}
                        {calculateMonthTotals(month).totalBookingFormCount}</Col>
                    </Row>
                  </Container>
                </div>
                {expandedRows.includes(month.month) && (
